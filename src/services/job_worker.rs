...
impl JobProcessor {
    ...
    async fn handle_deliver_webhook(&self, payload: &Value) -> Result<(), String> {
        ...
        let mut delivery = crate::models::WebhookDelivery::new(
        ...
        match response_result {
            Ok(response) => {
                ...
                if status.is_success() {
                    delivery.status = crate::models::DeliveryStatus::Success;
                    delivery.completed_at = Some(Utc::now().to_rfc3339());
                    info!("Webhook delivered successfully to {}", url);
                } else {
                    delivery.status = crate::models::DeliveryStatus::Failed;
