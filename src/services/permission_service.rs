...
impl PermissionService {
    ...
    pub fn has_any_permission(roles: &[Role], permissions: &[&str]) -> bool {
        permissions.iter().any(|perm| Self::has_permission(roles, perm))
    }
    ...
    pub fn has_all_permissions(roles: &[Role], permissions: &[&str]) -> bool {
        permissions.iter().all(|perm| Self::has_permission(roles, perm))
    }
...
mod tests {
    ...
    fn test_has_permission_single_role() {
        let roles = vec![create_test_role("Support Agent", vec![
    ...
    fn test_has_permission_multiple_roles() {
        let roles = vec![
            ...
            create_test_role("Manager", vec!["conversations:read_all", "conversations:update_team_assignee"]),
        ];
    ...
    fn test_has_any_permission() {
        let roles = vec![create_test_role("Support Agent", vec!["conversations:read_assigned"])];

        assert!(PermissionService::has_any_permission(&roles, &[
    ...
    fn test_has_all_permissions() {
        let roles = vec![create_test_role("Manager", vec![
            "conversations:read_all",
            "conversations:update_user_assignee",
            "conversations:update_team_assignee",
        ])];

        assert!(PermissionService::has_all_permissions(&roles, &[
            "conversations:read_all",
            "conversations:update_user_assignee",
        ]));
    ...
    fn test_get_all_permissions() {
        let roles = vec![
            create_test_role("Support Agent", vec!["messages:write", "conversations:read_assigned"]),
    ...
    fn test_empty_roles() {
        ...
        assert!(!PermissionService::has_any_permission(&roles, &["any:permission"]));
        assert!(!PermissionService::has_all_permissions(&roles, &["any:permission"]));
    }
}
