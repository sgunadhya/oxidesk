...
// Implement domain ApiKeyRepository trait for Database
#[async_trait::async_trait]
impl crate::domain::ports::api_key_repository::ApiKeyRepository for Database {
    async fn revoke_api_key(&self, agent_id: &str) -> ApiResult<bool> {
        // Delegate to the database ApiKeyRepository implementation
        <Self as crate::infrastructure::persistence::api_key::ApiKeyRepository>::revoke_api_key(self, agent_id).await
    }

    async fn count_api_keys(&self) -> ApiResult<i64> {
        // Delegate to the database ApiKeyRepository implementation
        <Self as crate::infrastructure::persistence::api_key::ApiKeyRepository>::count_api_keys(self).await
    }

    async fn get_agent_by_api_key(&self, api_key: &str) -> ApiResult<Option<Agent>> {
        // Delegate to the database ApiKeyRepository implementation
        <Self as crate::infrastructure::persistence::api_key::ApiKeyRepository>::get_agent_by_api_key(self, api_key).await
    }

    async fn update_api_key_last_used(&self, api_key: &str) -> ApiResult<()> {
        // Delegate to the database ApiKeyRepository implementation
        <Self as crate::infrastructure::persistence::api_key::ApiKeyRepository>::update_api_key_last_used(self, api_key).await
    }
    ...
    async fn create_api_key(
        ...
        <Self as crate::infrastructure::persistence::api_key::ApiKeyRepository>::create_api_key(self, agent_id, api_key, api_secret_hash, desc_str).await
    }
    ...
    async fn list_api_keys(
        ...
        // Delegate to the database ApiKeyRepository implementation
        let result = <Self as crate::infrastructure::persistence::api_key::ApiKeyRepository>::list_api_keys(self, limit, offset, sort_by, sort_order).await?;
        // Convert description from String to Option<String>
        Ok(result.into_iter().map(|(agent_id, api_key, description, created_at, last_used_at)| {
            (agent_id, api_key, Some(description), created_at, last_used_at)
        }).collect())
    }
}
