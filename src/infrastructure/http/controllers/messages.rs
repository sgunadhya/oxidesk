...
use crate::{
    ...
    domain::entities::{IncomingMessageRequest, MessageListResponse, PaginationMetadata, SendMessageRequest},
};
    ...
    if request.contact_id.is_none() {
        if let Some(ref from_header) = request.from_header {
            ...
            let (first_name, last_name, email) =
                crate::application::services::user_service::parse_email_display_name(from_header);
            ...
            return Err(crate::infrastructure::http::middleware::ApiError::BadRequest(
