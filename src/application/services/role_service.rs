...
pub type RoleService = RoleDomainService;
...
impl RoleDomainService {
    ...
    pub async fn list_permissions(&self) -> DomainResult<Vec<crate::domain::entities::Permission>> {
    ...
    pub async fn delete(&self, _auth_user: &crate::domain::entities::User, id: &str) -> DomainResult<()> {
        self.delete_role(id).await
    }

    pub async fn list_roles_raw(&self) -> DomainResult<Vec<Role>> {
        self.list_roles().await
    }

    pub async fn get_role_permissions(&self, role_id: &str) -> DomainResult<Vec<crate::domain::entities::Permission>> {
        let role = self.get_role(role_id).await?;
        // Return permissions for this role
        let all_permissions = self.list_permissions().await?;
        Ok(all_permissions.into_iter().filter(|p| {
            role.permissions.contains(&p.id)
        }).collect())
    }
}
