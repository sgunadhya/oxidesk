...
use crate::{
    infrastructure::http::middleware::ApiError,
    domain::entities::{AuthMethod, OidcProvider, Session, User, UserType},
};
use std::sync::Arc;
use openidconnect::{
...
pub struct CallbackResult {
    pub session: Session,
    pub user: User,
    pub agent: crate::domain::entities::Agent,
    pub roles: Vec<crate::domain::entities::Role>,
}
...
impl OidcService {
    ...
    pub async fn create_state(&self, state: &crate::domain::entities::OidcState) -> Result<(), ApiError> {
        self.oidc_repo.create_state(state).await
    }

    /// Consume OIDC state (one-time use)
    pub async fn consume_state(&self, state: &str) -> Result<Option<crate::domain::entities::OidcState>, ApiError> {
    ...
    pub async fn handle_callback(
        ...
        let roles: Vec<crate::domain::entities::Role> = domain_roles
            .into_iter()
            .map(|r| crate::domain::entities::Role {
                id: r.id,
                name: r.name,
        ...
        let token = crate::application::services::auth::generate_session_token();
