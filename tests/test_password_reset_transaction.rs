...
async fn test_weak_password_does_not_mark_token_as_used() {
    ...
    let result = PasswordResetService::new(oxidesk::domain::ports::password_reset_repository::PasswordResetRepository::new(db.clone()), std::sync::Arc::new(db.clone())).reset_password( token, "weak").await;
    ...
    let result2 = PasswordResetService::new(oxidesk::domain::ports::password_reset_repository::PasswordResetRepository::new(db.clone()), std::sync::Arc::new(db.clone())).reset_password( token, "StrongPass123!").await;
    assert!(
        result2.is_ok(),
...
async fn test_used_token_does_not_change_password_again() {
    ...
    // Use token once
    PasswordResetService::new(oxidesk::domain::ports::password_reset_repository::PasswordResetRepository::new(db.clone()), std::sync::Arc::new(db.clone())).reset_password( token, "FirstNewPass123!")
        .await
        .unwrap();
    ...
    let result = PasswordResetService::new(oxidesk::domain::ports::password_reset_repository::PasswordResetRepository::new(db.clone()), std::sync::Arc::new(db.clone())).reset_password( token, "SecondNewPass123!").await;
