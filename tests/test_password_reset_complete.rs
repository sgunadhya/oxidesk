...
async fn test_successful_password_reset_with_valid_token() {
    ...
    let agent = Agent::new(user.id.clone(), "Reset Test".to_string(), None, old_password_hash.clone());
...
async fn test_password_reset_marks_token_as_used() {
    ...
    let user = User::new(email.clone(), UserType::Agent);
    let agent = Agent::new(user.id.clone(), "Token Used Test".to_string(), None, password_hash);
...
async fn test_password_reset_rejects_invalid_token_format() {
    ...
    for invalid_token in &invalid_tokens {
        ...
        assert!(result.is_err(), "Should reject invalid token format: {}", invalid_token);

        match result.unwrap_err() {
            ApiError::BadRequest(msg) => {
                assert_eq!(msg, "Invalid token format");
            },
            other => panic!("Expected BadRequest for invalid token format, got: {:?}", other),
        }
    }
...
async fn test_password_reset_rejects_nonexistent_token() {
    ...
    match result.unwrap_err() {
        ...
        other => panic!("Expected BadRequest for non-existent token, got: {:?}", other),
    }
...
async fn test_password_reset_rejects_weak_passwords() {
    ...
    let user = User::new(email.clone(), UserType::Agent);
    let agent = Agent::new(user.id.clone(), "Weak Pass Test".to_string(), None, password_hash);
...
async fn test_password_reset_updates_password_hash() {
    ...
    let user = User::new(email.clone(), UserType::Agent);
    let agent = Agent::new(user.id.clone(), "Hash Update Test".to_string(), None, old_password_hash.clone());
    ...
    // Reset password
    let new_password = "NewPass123!";
    password_reset_service::reset_password(db, token, new_password).await.unwrap();
...
async fn test_password_reset_returns_success_message() {
    ...
    let user = User::new(email.clone(), UserType::Agent);
    let agent = Agent::new(user.id.clone(), "Message Test".to_string(), None, password_hash);
