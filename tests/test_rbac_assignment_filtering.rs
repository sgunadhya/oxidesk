...
use helpers::rbac_helpers::{
    create_test_agent as create_rbac_test_agent,
    create_test_role,
    create_auth_user_with_roles,
    create_test_team,
    add_user_to_team,
    ensure_test_inbox,
    create_conversation_assigned_to_user,
    create_conversation_assigned_to_team,
};
...
async fn test_assigned_conversation_access_granted() {
    ...
    let conv_id = create_conversation_assigned_to_user(
        db,
        &contact.id,
        &alice.user.id,
    )
    ...
    let conversation = db.get_conversation_by_id(&conv_id).await
        .expect("Failed to get conversation")
        .expect("Conversation not found");

    let has_access = conversation.assigned_user_id.as_ref() == Some(&alice.user.id);

    assert!(has_access, "Alice should have access to her assigned conversation");
}
...
async fn test_unassigned_conversation_access_denied() {
    ...
    assert!(!has_access, "Alice should not have access to Bob's conversation");
}
...
async fn test_admin_with_read_all_bypasses_assignment() {
    ...
    // Create manager
    let manager = create_auth_user_with_roles(
        db,
        "manager@example.com",
        "Manager",
        vec![manager_role],
...
async fn test_team_assignment_grants_access() {
    ...
    // Create agent Dave
    let dave = create_auth_user_with_roles(
        db,
        "dave@example.com",
        "Dave",
        vec![support_role],
    ...
    let conv_id = create_conversation_assigned_to_team(
        db,
        &contact.id,
        &team_id,
    )
    ...
    let user_teams = db.get_user_teams(&dave.user.id).await.expect("Failed to get user teams");
    let has_access = conversation.assigned_team_id.as_ref()
...
async fn test_non_team_member_denied_team_conversation() {
    ...
    let user_teams = db.get_user_teams(&alice.user.id).await.expect("Failed to get user teams");
    let has_access = conversation.assigned_team_id.as_ref()
        .map_or(false, |team_id| user_teams.iter().any(|t| &t.id == team_id));

    assert!(!has_access, "Alice should not have access to team conversation (not a member)");
}
...
async fn test_user_and_team_assignment_both_work() {
    ...
    // Create agent
    let agent = create_auth_user_with_roles(
        db,
        "agent@example.com",
        "Agent",
        vec![support_role],
    ...
    let conv_user = create_conversation_assigned_to_user(
        db,
        &contact.id,
        &agent.user.id,
    )
    ...
    // Create conversation assigned to team
    let conv_team = create_conversation_assigned_to_team(
        db,
        &contact.id,
        &team_id,
    )
    ...
    let conv_user_obj = db.get_conversation_by_id(&conv_user).await
        .expect("Failed to get conversation")
        .expect("Conversation not found");
    let has_access_user = conv_user_obj.assigned_user_id.as_ref() == Some(&agent.user.id);

    assert!(has_access_user, "Agent should have access to directly assigned conversation");

    // Should have access to team-assigned conversation (check via database)
    let conv_team_obj = db.get_conversation_by_id(&conv_team).await
        .expect("Failed to get conversation")
        .expect("Conversation not found");
    let user_teams = db.get_user_teams(&agent.user.id).await.expect("Failed to get user teams");
    let has_access_team = conv_team_obj.assigned_team_id.as_ref()
        .map_or(false, |team_id| user_teams.iter().any(|t| &t.id == team_id));

    assert!(has_access_team, "Agent should have access to team-assigned conversation");
}
...
async fn test_unassigned_conversation_no_access() {
    ...
    let user_teams = db.get_user_teams(&agent.user.id).await.expect("Failed to get user teams");
    let has_team_assignment = conversation.assigned_team_id.as_ref()
        .map_or(false, |team_id| user_teams.iter().any(|t| &t.id == team_id));
    let has_access = has_user_assignment || has_team_assignment;

    assert!(!has_access, "Agent should not have access to unassigned conversation");
}
