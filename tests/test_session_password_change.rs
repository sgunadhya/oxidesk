...
async fn test_password_change_destroys_all_user_sessions() {
    ...
    let admin_agent = Agent::new(admin_user.id.clone(), "Admin User".to_string(), None, admin_password_hash.clone());
    ...
    let agent = Agent::new(user.id.clone(), "Test Agent".to_string(), None, password_hash);
    ...
    assert_eq!(sessions_before.len(), 3, "Should have 3 sessions before password change");
    ...
    assert_eq!(sessions_after.len(), 0, "All sessions should be destroyed after password change");
...
async fn test_password_change_requires_reauthentication() {
    ...
    let admin_password_hash = hash_password("AdminPass123!").unwrap();
    let admin_user = User::new(admin_email.clone(), UserType::Agent);
    let admin_agent = Agent::new(admin_user.id.clone(), "Admin User 2".to_string(), None, admin_password_hash.clone());
    ...
    let password_hash = hash_password("OldPass123!").unwrap();
    let user = User::new(email.clone(), UserType::Agent);
    let agent = Agent::new(user.id.clone(), "Test Agent 2".to_string(), None, password_hash);
    ...
    let session_before = db.get_session_by_token(&old_token).await
        .map_err(|e| {
            eprintln!("Error getting session: {:?}", e);
            e
        })
        .unwrap();
    assert!(session_before.is_some(), "Session should exist before password change");
    ...
    agent_service::change_agent_password(db, &auth_user, &user.id, request).await.unwrap();

    // Verify old session no longer exists
    let session_after = db.get_session_by_token(&old_token).await.unwrap();
    assert!(session_after.is_none(), "Old session should not exist after password change");
...
async fn test_password_change_does_not_affect_other_users() {
    ...
    let admin_password_hash = hash_password("AdminPass123!").unwrap();
    let admin_user = User::new(admin_email.clone(), UserType::Agent);
    let admin_agent = Agent::new(admin_user.id.clone(), "Admin User 3".to_string(), None, admin_password_hash.clone());
    ...
    let agent2 = Agent::new(user2.id.clone(), "Agent 2".to_string(), None, password_hash.clone());
    ...
    agent_service::change_agent_password(db, &auth_user, &user1.id, request).await.unwrap();

    // Verify agent1 sessions are destroyed
    let agent1_sessions = db.get_user_sessions(&user1.id).await.unwrap();
    assert_eq!(agent1_sessions.len(), 0, "Agent 1 sessions should be destroyed");

    // Verify agent2 sessions are NOT affected
    let agent2_sessions = db.get_user_sessions(&user2.id).await.unwrap();
    assert_eq!(agent2_sessions.len(), 1, "Agent 2 sessions should NOT be affected");
...
async fn test_password_change_with_no_active_sessions() {
    ...
    let admin_password_hash = hash_password("AdminPass123!").unwrap();
    let admin_user = User::new(admin_email.clone(), UserType::Agent);
    let admin_agent = Agent::new(admin_user.id.clone(), "Admin User 4".to_string(), None, admin_password_hash.clone());
    ...
    let password_hash = hash_password("OldPass123!").unwrap();
    let user = User::new(email.clone(), UserType::Agent);
    let agent = Agent::new(user.id.clone(), "Agent No Sessions".to_string(), None, password_hash);
    ...
    let result = agent_service::change_agent_password(db, &auth_user, &user.id, request).await;
    assert!(result.is_ok(), "Password change should succeed even with no active sessions");
...
async fn test_password_change_permission_required() {
    ...
    let agent = Agent::new(agent_user.id.clone(), "Regular Agent".to_string(), None, agent_password_hash.clone());
    ...
    let target_agent = Agent::new(target_user.id.clone(), "Target Agent".to_string(), None, target_password_hash);
    ...
    let result = agent_service::change_agent_password(db, &auth_user, &target_user.id, request).await;
