...
async fn test_email_enumeration_prevention_same_message() {
    ...
    let result1 = PasswordResetService::new(db.clone()).request_password_reset(&existing_email).await;
...
async fn test_email_enumeration_prevention_consistent_timing() {
    ...
    PasswordResetService::new(db.clone()).request_password_reset(&existing_email)
        .await
        .unwrap();
    ...
    PasswordResetService::new(db.clone()).request_password_reset("nonexistent@example.com")
        .await
        .unwrap();
...
async fn test_no_token_created_for_nonexistent_email() {
    ...
    // Request password reset for non-existent email
    let result = PasswordResetService::new(db.clone()).request_password_reset("ghost@example.com").await;
    assert!(result.is_ok());
...
async fn test_email_enumeration_prevention_case_insensitive() {
    ...
    for email_variant in &variations {
        let result = PasswordResetService::new(db.clone()).request_password_reset(email_variant).await;
        assert!(result.is_ok());
