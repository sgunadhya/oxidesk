...
async fn test_lazy_cleanup_only_on_expired_not_used() {
    ...
    PasswordResetService::new(db.clone()).reset_password(&token_value, "NewPass123!")
        .await
        .unwrap();
    ...
    let result = PasswordResetService::new(db.clone()).reset_password(&token_value, "AnotherPass123!").await;
...
async fn test_cleanup_does_not_affect_other_users_tokens() {
    ...
    PasswordResetService::new(db.clone()).request_password_reset(&email1)
        .await
        .unwrap();
    PasswordResetService::new(db.clone()).request_password_reset(&email2)
        .await
        .unwrap();
    ...
    let result1 = PasswordResetService::new(db.clone()).reset_password(&token1_value, "NewPass123!").await;
...
async fn test_no_background_cleanup_job() {
    ...
    let _ = PasswordResetService::new(db.clone()).reset_password(&token_value, "NewPass123!").await;
